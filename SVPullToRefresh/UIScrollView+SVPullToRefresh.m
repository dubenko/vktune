...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"PULL_TO_REFRESH",),
                                                       NSLocalizedString(@"RELEASE_TO_REFRESH",),
                                                       NSLocalizedString(@"LOADING_TO_REFRESH",),
                                                       nil];
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        self.arrow.center = CGPointMake(self.arrow.bounds.size.width/2, self.arrow.bounds.size.height/2);
...
- (SVPullToRefreshArrow *)arrow {
    if(!_arrow) {
		_arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height, 22, 33)];
...
- (void)drawRect:(CGRect)rect {
    [self addSubview:[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Refresh_Up.png"]] ];
    /*
