...
+(MBHUDView*)hudWithBody:(NSString*)body type:(MBAlertViewHUDType)type hidesAfter:(float)delay show:(BOOL)show
{
    ...
    alert.backgroundColor = [UIColor colorWithWhite:0.0 alpha:1];
...
-(void)loadView
{
    ...
    self.modalBackground.alpha = (_backgroundAlpha > 0 ? _backgroundAlpha : 0.95);
...
-(void)layoutView
{
    ...
    if(_imageView)
    {
        ...
        rect.origin = CGPointMake(contentFrame.size.width/2-self.imageView.frame.size.width/2, 0);
